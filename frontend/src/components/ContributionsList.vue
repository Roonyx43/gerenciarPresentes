<template>
  <div>
    ...
    <div class="mb-4 grid md:grid-cols-3 gap-3 ">
      <input v-model="search" type="text" placeholder="Buscar por padrinho, txid..." class="input border-2 border-blue-300 rounded-sm p-4" />
      <select v-model="status" class="input border-2 border-blue-300 rounded-sm p-4">
        <option value="">Todos os status</option>
        <option value="pending">pending</option>
        <option value="paid">paid</option>
        <option value="canceled">canceled</option>
      </select>
      <button class="btn btn-ghost bg-blue-400 rounded-sm color-white text-white cursor-pointer" @click="reload" :disabled="loading">
    ...
    <div class="overflow-auto rounded-sm shadow border-2 p-3 border-blue-300">
      <table class="min-w-full text-sm">
        ...
        <tbody>
          <tr v-if="loading">
            <td colspan="7" class="p-6 text-center text-gray-500">Carregando contributions...</td>
          </tr>

          <tr v-for="c in filtered" :key="c.id" class="border-t bg-white/60 backdrop-blur">
            <td class="p-3">{{ c.id }}</td>
            <td class="p-3">
              <div class="font-medium">{{ c.padrinho_name }}</div>
              <div class="text-xs text-gray-500">txid: {{ c.txid }}</div>
            </td>
            <td class="p-3">R$ {{ fix2(c.amount) }}</td>
            <td class="p-3">{{ c.gift_name || '-' }}</td>
            <td class="p-3">
              <span :class="badgeClass(c.status)" class="badge">
                {{ c.status }}
              </span>
            </td>
    ...
    <Modal :open="modal.open" title="Excluir contribution" confirm-text="Excluir"
           @close="modal.open=false" @confirm="removeContribution(modal.id)">
      Se a contribuição estava paga, o valor será abatido do gift vinculado.
    </Modal>
  </div>
</template>
...
import { listContributions, toggleContributionPaid, deleteContribution } from '../api';
...
const badgeClass = (s) => {
  ...
  if (s === 'pending') return 'border-yellow-200 bg-yellow-50 text-yellow-700';
  if (s === 'canceled') return 'border-gray-200 bg-gray-50 text-gray-600';
